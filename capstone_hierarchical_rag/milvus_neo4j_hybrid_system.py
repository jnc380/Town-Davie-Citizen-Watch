...
class MilvusOnlyRAGSystem:
	def __init__(self):
		...
		# Feature flags
		self.use_graph_hints = os.getenv("USE_GRAPH_HINTS", "true").lower() == "true"
		...
		self._concept_lexicon: Dict[str, str] = {}  # name/slug -> slug
		
		# Neo4j HTTP Query API
		self.neo4j_api_url = os.getenv("NEO4J_QUERY_API_URL")
		self.neo4j_user = os.getenv("NEO4J_USERNAME")
		self.neo4j_pass = os.getenv("NEO4J_PASSWORD")
		self._neo4j_http_ready = bool(self.neo4j_api_url and self.neo4j_user and self.neo4j_pass and httpx)
		if self.use_graph_hints and self._neo4j_http_ready:
			self._load_concept_lexicon_http()
	...
	def _http_headers(self) -> Dict[str, str]:
		token = base64.b64encode(f"{self.neo4j_user}:{self.neo4j_pass}".encode("utf-8")).decode("utf-8")
		return {"Authorization": f"Basic {token}", "Content-Type": "application/json", "Accept": "application/json"}

	def _neo4j_http_query(self, statement: str, parameters: Optional[Dict[str, Any]] = None) -> List[List[Any]]:
		if not self._neo4j_http_ready:
			return []
		try:
			payload = {"statement": statement, "parameters": parameters or {}}
			rows: List[List[Any]] = []
			with httpx.Client(timeout=15) as client:
				resp = client.post(self.neo4j_api_url, json=payload, headers=self._http_headers())
				if resp.status_code not in (200, 202):
					logger.warning(f"Neo4j HTTP status={resp.status_code}")
					return []
				body = resp.json() or {}
				data = body.get("data")
			...
			logger.warning(f"Neo4j HTTP query failed: {e}")
			return []

	def _load_concept_lexicon_http(self) -> None:
		rows = self._neo4j_http_query("MATCH (c:Concept) RETURN c.slug AS slug, c.name AS name")
		lex: Dict[str, str] = {}
		for row in rows:
			try:
				slug = (row[0] if len(row)>0 else "") or ""
				name = (row[1] if len(row)>1 else "") or ""
				slug = str(slug).strip(); name = str(name).strip()
				if slug:
					lex[slug.lower()] = slug
					if name:
						lex[name.lower()] = slug
				...
				continue
		self._concept_lexicon = lex
		logger.info(f"Loaded {len(lex)} concept lexicon entries from Neo4j (HTTP)")

	def _extract_concepts(self, query: str) -> List[str]:
		q = (query or "").lower()
		out: List[str] = []
		for key, slug in self._concept_lexicon.items():
			if key and key in q:
				out.append(slug)
	...
	def _expand_via_graph_http(self, concept_slugs: List[str], top_k: int = 50) -> Tuple[List[str], List[str]]:
		if not (self._neo4j_http_ready and concept_slugs):
			return ([], [])
		stmt = (
			"MATCH (seed:Concept) WHERE seed.slug IN $slugs "
			"OPTIONAL MATCH (seed)-[:PARENT_OF*1..3]->(c1:Concept) "
			"WITH seed, collect(DISTINCT c1) + seed AS cs "
			"UNWIND cs AS c "
			"MATCH (a:AgendaItem)-[hc:HAS_CONCEPT]->(c) "
			"OPTIONAL MATCH (m:Meeting)-[:HAS_AGENDA_ITEM]->(a) "
			"WITH a, m "
			"RETURN collect(DISTINCT m.meeting_id)[0..$k] AS mids, collect(DISTINCT a.title)[0..10] AS titles"
		)
		rows = self._neo4j_http_query(stmt, {"slugs": concept_slugs, "k": int(top_k)})
		try:
			row = rows[0] if rows else []
			mids = row[0] if len(row) > 0 else []
			titles = row[1] if len(row) > 1 else []
			mids = [str(x) for x in (mids or []) if x]
	...
	async def refresh_collection_date_range(self) -> Tuple[Optional[str], Optional[str]]:
		"""Refresh cached collection date range; fallback to video-only, then file-based if needed."""
		...
		if not (cmin and cmax):
			...
			try:
				...
				minmax_path = base / "meeting_dates_minmax.txt"
				if minmax_path.exists():
					lines = minmax_path.read_text(encoding="utf-8").splitlines()
					vals: Dict[str, str] = {}
					for ln in lines:
						parts = [p.strip() for p in ln.split("\t")]
						if len(parts) >= 2:
							vals[parts[0].lower()] = parts[1]
					cmin = vals.get("min") or cmin
					cmax = vals.get("max") or cmax
			except Exception:
				pass
		if cmin and cmax:
	...
	async def hybrid_search(self, query: str, limit: int = None) -> List[SearchResult]:
		try:
			...
			# Broad Milvus
			broad_dense = await self.search_milvus_dense(query, dense_limit)
			broad_sparse = await self.search_milvus_sparse(query, sparse_limit)
			combined: Dict[str, SearchResult] = {}
			for r in broad_dense: combined[r.chunk_id] = r
			for r in broad_sparse: combined[r.chunk_id] = r
			# Graph-guided
			if self.use_graph_hints and self._neo4j_http_ready:
				slugs = self._extract_concepts(query)
				mids, titles = self._expand_via_graph_http(slugs)
				if mids:
					aug_query = (query + " " + " ".join(titles[:3])).strip()
					try:
						embedding_response = await self.openai_client.embeddings.create(model="text-embedding-3-small", input=aug_query)
						qemb = embedding_response.data[0].embedding
						expr = f"meeting_id in [{','.join(f'\"{m}\"' for m in mids[:50])}]"
						flt = await self._zilliz_search(qemb, dense_limit, expr)
						for hit in flt:
							md = hit.get("metadata") or {}
							combined[hit.get("chunk_id")] = SearchResult(
								chunk_id=hit.get("chunk_id",""), content=hit.get("content",""), document_type=str(hit.get("type","")),
								meeting_id=str(hit.get("meeting_id","")), meeting_date=str(hit.get("meeting_date","")), meeting_type=str(hit.get("meeting_type","")),
								hierarchy=md.get("hierarchy",""), section_header=md.get("section_header",""), metadata=md, score=float(hit.get("score",0.0)), search_type="dense_filtered")
					except Exception as e:
						logger.warning(f"Graph-filtered search failed: {e}")
