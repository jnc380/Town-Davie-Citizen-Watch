#!/usr/bin/env python3
"""
Milvus-Only RAG System using PyMilvus SDK
...
class MilvusPyMilvusRAGSystem:
    ...
    def _connect_milvus(self):
        ...
        try:
            logger.info("Connecting to Zilliz (Milvus) via PyMilvus...")
                ...
                for hit in hits:
                    try:
                        ...
                        search_results.append(SearchResult(
                            ...
                            section_header=hit.entity.get("section_header", ""),
                            metadata=metadata,
                            score=hit.score,
                            search_type="dense"
                        ))
    ...
    async def search_milvus_sparse(self, query: str, limit: int = 20) -> List[SearchResult]:
        """Perform sparse search using BM25 (simulated with text search)"""
        try:
            # For sparse search, we'll use a simple text-based approach
            # In a real implementation, you might implement BM25 or use Milvus's built-in text search
            
            # Load collection
            self.collection.load()
            
            # Use expression to search for query terms in content
            query_terms = query.lower().split()
            expressions = []
            for term in query_terms:
                if len(term) > 2:  # Only search for terms longer than 2 characters
                    expressions.append(f'content like "%{term}%"')
...
# FastAPI app
app = FastAPI(
    title="Town of Davie RAG System (Milvus PyMilvus)",
...
async def root(request: Request):
    ...
    return {"message": "Milvus PyMilvus RAG system is running"}
